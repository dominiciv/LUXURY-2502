...
export default function RedirectButton() {
  const targetUrl =
    "https://api.whatsapp.com/send/?phone=+5491162057482&text=Hola!+Mi+nombre+es+______+Vi+este+anuncio%2C+quiero+un+usuario+por+favor!+gracias.&type=phone_number&app_absent=0"

  return (
    <motion.div
      animate={{ y: [0, -10, 0] }} // Rebote arriba y abajo
      transition={{
        repeat: Infinity, // Loop infinito
        repeatType: "loop",
        duration: 0.6, // Duración de cada ciclo de rebote
        ease: "easeInOut",
      }}
    >
      <Button
        onClick={() => (window.location.href = targetUrl)}
        className="relative bg-transparent font-bold transition-all duration-300 ease-in-out transform hover:scale-110 border-2 border-transparent hover:border-golden hover:shadow-golden group h-[60px] w-[320px]"
      >
